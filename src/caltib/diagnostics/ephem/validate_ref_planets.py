...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    p.add_argument("--out-png", default="planetary_validation.png")
    ...
    for jd in jds:
        ...
        # Using .flatten() to convert shape (3, 1) into (3,) 
        ...
        for p_name in planets_to_test:
            ...
            # 6. Calculate Residuals in Arcseconds
    ...
    for i, p_name in enumerate(planets_to_test):
        ...
        axs[i].set_title(f"{p_name.capitalize()} True Geocentric Longitude Error (Micro-VSOP - DE422)")
        axs[i].set_ylabel("Error (arcsec)")
        axs[i].grid(True, alpha=0.3)
        
        # Add a light baseline to visually anchor the zero-error line
        axs[i].axhline(0, color='black', linewidth=0.8, alpha=0.8)

    axs[-1].set_xlabel("Year")
    
    plt.suptitle(f"Planetary Geocentric Validation against DE422 ({years[0]:.0f} to {years[-1]:.0f})", fontsize=14)
