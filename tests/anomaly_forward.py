...
def forward_anomaly_series(np, engine: str, jd_start: float, jd_end: float, step_days: float) -> Optional[SeriesData]:
    ...
    # Center to exactly zero-mean to align perfectly with DE422 detrending
...
def main(argv: Optional[List[str]] = None) -> int:
    p = argparse.ArgumentParser(description="Forward kinematic anomaly vs DE422.")
    ...
    p.add_argument("--step", type=float, default=0.25, help="DE422 and forward sampling step in days.")
    ...
    # Forward proxy series for L1-L4
    for eng in engines:
        try:
    ...
    for s in series:
        ...
        if s.label == "DE422":
            plt.plot(x_rel, s.anomaly_deg, linestyle="--", color=c, linewidth=1.5, alpha=0.9, label=s.label)
        else:
            plt.plot(x_rel, s.anomaly_deg, linestyle="-", color=c, linewidth=1.2, alpha=0.8, label=s.label)

    plt.title("Forward Kinematic Anomaly (True - Mean Elongation)\nContinuous Physical Time vs DE422 Ephemeris")
